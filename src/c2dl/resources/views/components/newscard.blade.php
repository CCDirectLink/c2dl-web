<article class="c2dl-newscard">
    <div class="c2dl-newscard-title" itemscope itemtype="http://schema.org/Article" role="banner">
        ...
        <div class="c2dl-newscard-info">
            ...
            <time datetime="{{ $entry->created('c', 'UTC', 'GR') }}" itemprop="datePublished" class="c2dl-newscard-info-elem c2dl-newscard-created" name="time_created">{!! $entry->created(null, 'UTC', 'HE.HTML') !!}</time>
            @if ($entry->is_updated())
                <time datetime="{{ $entry->updated('c', 'UTC', 'GR') }}" itemprop="dateModified" class="c2dl-newscard-info-elem c2dl-newscard-updated" name="time_modified">(last update {!! $entry->updated(null, 'UTC', 'HE.HTML') !!})</time>
            @endif
        </div>
    </div>
